...
{
    public class NodeActionSeed : NodeActionBase
    {
        private readonly string m_varCrop = "crop";

        private readonly string m_varWhere = "cropLocation";

        public NodeActionSeed() {}

        public NodeActionSeed(string varCrop, string varWhere)
        {
            m_varCrop  = varCrop;
            m_varWhere = varWhere;
        }
        ...
        public override NodeState OnUpdate()
        {
            ...
            var msg = Context.World.Send(AgentState.eid, new Msg.ActionSeed() { target = crop, where  = where });
